...
- **Scratch dump (mandatory for spreadsheets):** `research/GCC/brainstorm-analysis/_scratch-brainstorm-sources-GCC-E2E-027.md` (produced via `python3 scripts/dump_research_folder_to_text.py research/GCC/brainstorm-sessions -o research/GCC/brainstorm-analysis/_scratch-brainstorm-sources-GCC-E2E-027.md`; dependencies: `python3 -m pip install -r scripts/requirements-research-xlsx.txt`)
- **Files excluded (region mismatch):** None (only one in-scope file in folder; `.gitkeep` ignored by dump script)
- **Dashboard filter applied in export (re-read from sheets this run):** Product Features Model: **Talent Management → Talent Acquisition**, **N = 9,922** idea records. Unfiltered product-area baseline **N = 51,007** (Filter Page by Product Area).
- **Completed (UTC):** 2026-03-27T15:45:00Z (analysis run; export timestamp on sheets **03/11/2026 9:59 AM**)

---

## Executive summary

- **9,922** P&T ideas sit under the **Talent Acquisition** product-area filter in this export. On **Sentiment: Product Area (Model Topics)** (Talent Acquisition aggregate), **negative** bucket share is **~0.527** with **5,233** ideas and modelled sentiment **~-1.918** (positive bucket **~0.395**, **3,917** ideas, **~1.964**); **neutral** bucket **8,530** ideas at **~-0.001** sentiment; overall **Talent Acquisition** row shows **N = 9,922** and **~-0.164** aggregate sentiment (dashboard semantics: confirm field definitions before executive numeric claims).
...
- **Idea Question Responses** (first rows ingested in scratch dump this run) includes recurring themes such as **per-question job application questionnaires**, **comp package / job catalog** adjustments, and **learning / certification** tracking gaps; keyword scans across the **2,000-row-per-sheet** dump reinforce configurability, **requisition**, **application**, and **candidate** language — overlap with **105** GCC narratives is **possible** but **not proven** by this file alone.

## Session inventory

| Session / artefact | Type | Date (export) | Participants | Notes |
|--------------------|------|---------------|--------------|-------|
| `P&T Idea Results Dashboard_03_11_2026 9_59 AM.xlsx` | Qualtrics P&T dashboard export (multi-sheet) | 03/11/2026 9:59 AM on sheets | Owners **pavithra.krishna@workday.com** (widgets); **cynthia.johnson@workday.com** (filter page) | **25** sheets in workbook; **9,922** under TA linked filter; **51,007** unfiltered product-area view |
...
**Evidence:** Representative verbatim fragments are in the **Idea Question Responses** sheet (see scratch dump; **2,000 rows per sheet** cap in `dump_research_folder_to_text.py`).
...
From sheet **Product Capability Volume, Sentiment and Effort (Model Topics)** (Talent Management → Talent Acquisition, **N = 9,922**). Values re-read from workbook **27 March 2026** (this run; rounded for readability).
...
| Mass Action Capabilities | 366 | -0.117 | -0.913 |
| Screening and Assessments | 354 | -0.118 | -0.521 |
| Referrals | 202 | -0.205 | -1.473 |
| Background Checks and References | 199 | -0.221 | -0.459 |
| Agencies | 173 | -0.267 | -1.094 |
| Browse Jobs | 152 | -0.149 | -1.580 |
| Diversity, Equity and Inclusion | 94 | -0.165 | -0.400 |
...
## Appendix: Talent Acquisition sentiment bands (dashboard)

From sheet **Sentiment: Product Area (Model Topics)** (aggregate row **N = 9,922**, sentiment **~-0.164**). Stacked-band rows re-read this run: **Neutral** **8,530** (**~0.860** % Parent), **Negative** **5,233** (**~0.527**), **Positive** **3,917** (**~0.395**). **Do not** treat band volumes as mutually exclusive counts that sum to **9,922** (Qualtrics stacked sentiment semantics; confirm before executive claims).
