...
- **Scratch dump (mandatory for spreadsheets):** `research/GCC/brainstorm-analysis/_scratch-brainstorm-sources-GCC-E2E-026.md` (produced via `python3 scripts/dump_research_folder_to_text.py research/GCC/brainstorm-sessions -o …`; dependencies: `python3 -m pip install -r scripts/requirements-research-xlsx.txt`)
...
- **Completed (UTC):** 2026-03-27T12:00:00Z (analysis run; export timestamp on sheets **03/11/2026 9:59 AM**)

---

## Executive summary

- **9,922** P&T ideas sit under the **Talent Acquisition** product-area filter in this export. On **Sentiment- Product Area (Model Topics)** (Talent Acquisition aggregate), **negative** bucket share is **~0.527** with **5,233** ideas and modelled sentiment **~-1.918** (positive bucket **~0.395**, **3,917** ideas, **~1.964**); overall **neutral** row shows **N = 9,922** and **~-0.164** sentiment (dashboard semantics: confirm field definitions before executive numeric claims).
...
- Keyword scans of **Idea Question Responses** (this run, full sheet ingested for counts) surface recurring internal themes: **questionnaires / application flow**, **candidate pools and delete**, **progress indicator**, **grid / columns**, **offers**, **custom fields / requisition templates**, and **DocuSign** — overlap with **105** GCC narratives is **possible** but **not proven** by this file alone.
...
**Evidence:** Representative verbatim fragments are in the **Idea Question Responses** sheet (see scratch dump and keyword extraction this run).
...
From sheet **Product Capability Volume, Sent** (Talent Management → Talent Acquisition, **N = 9,922**). Values re-read from workbook **27 March 2026** (this run; rounded for readability).
