...
**Scratch dump (this run):** `research/GCC/win-loss-analysis/_scratch-winloss-sources-GCC-E2E-028.md`
...
| Source | Rows | Notes |
| --- | ---: | --- |
| `Opportunity Detail.xlsx` (Sheet 1) | 598 | Full export ingested; columns include `Gap Name`, `Opportunity`, `Deal Motion`, `Opportunity Stage `, `Opp Pipeline Stage`, `CI Notes Header`, `Pain point(s)`, `Country Specific Gap Detail`, `Industry Specific Gap Detail`, etc. |
...
| --- | --- | --- |
| PG-00009165 (`Gap ID` aTEVT0000001ueT4AQ) | North America | **Pain point(s)** references **GCC High** in the **Microsoft** compliance sense (integrations with Outlook, MS Teams, HiredScore), not Gulf states. |
...
**Win-Loss (107) column for GCC-E2E-028:**
...
• **Cross-check:** If **120** cites competitive dynamics, prefer **101** Step 1 scan and matrix changelog for **GCC-E2E-028** over win-loss alone.
...
- **Win-loss source files read (this run):**  
  - `research/GCC/win-loss-interviews/Opportunity Detail.xlsx` (Tier 1: folder match; **598** rows on Sheet 1; scratch: `research/GCC/win-loss-analysis/_scratch-winloss-sources-GCC-E2E-028.md`)  
  - `research/GCC/win-loss-interviews/.gitkeep` (ignored; no transcript content)  
- **Row-level filter (Gulf GCC):**  
  - Keyword scan across combined text from: `Country Specific Gap Detail`, `Gap Name`, `Pain point(s)`, `Proposed Solution`, `Opportunity`, `Opp Segment` (export uses `CI Notes Header` only; no free-text CI notes body in this file) for: `gcc`, `saudi`, `uae`, `qatar`, `bahrain`, `kuwait`, `oman`, `gulf`, `emirates`, `dubai`, `riyadh` (case-insensitive), with heuristic exclusion when **Microsoft**/**Outlook**/**Teams**/**HiredScore** co-occur with **GCC High**.  
...
- **Completed (UTC):** 2026-03-27T18:45:00Z  
