...
**Date:** 26 March 2026  
**Mission:** GCC-E2E-021 (Step 2.75)  
**Analyst role:** Win-Loss (107) — buyer-reported gaps from opportunity tracking, not verified product fact.

## Scope and data caveat

- **Source:** `research/GCC/win-loss-interviews/Opportunity Detail.xlsx`, re-ingested this run via `python3 scripts/dump_research_folder_to_text.py research/GCC/win-loss-interviews -o research/GCC/win-loss-analysis/_scratch-winloss-sources.md` (flattened scratch read for thematic coding).
...
- **GCC signal (refined):** Case-insensitive scan of **Gap Name**, **CI Notes**, **Pain point(s)**, **Country Specific Gap Detail**, **Proposed Solution**, **Functionality Requirement**, and **Gap Summary** for GCC, Middle East, Saudi, UAE, KSA, Qatar, Kuwait, Gulf, MENA, Oman, Bahrain, Emirates, Kingdom yields **one** row with a **substantive GCC** reference: **PG-00009165** (Outlook / Teams / HiredScore scheduling for **GCC** populations). In **this** export, **PG-00005541** has empty **CI Notes**, so the historical **"amenable"** substring false positive for **MENA** does **not** recur; still treat naive substring matching on notes as **unsafe** for automation.
...
• **Deal stages (all rows):** **Won** 179, **Open** 136, **Do Nothing** 107, **Lost** 96, **Closed** 39, **34- Merged Opportunity** 28, **Qual Out** 13. The loss bucket is large enough to contrast with won on severity and themes.  
...
• **Competitor and adjacent product mentions** (buyer narrative in notes, all rows, keyword scan): **Paradox** 52, **Greenhouse** 43, **LinkedIn** 20, **Phenom** 18, **Indeed** 13, **iCIMS** 12, **SmartRecruiters** 7, **Broadbean** 6, **Lever** 6, plus sparser **Taleo**, **eArcu**, **SAP**, **SuccessFactors**, **Clinch**, **CareerPlug**. **Lost** rows alone show repeated **LinkedIn**, **Indeed**, **Greenhouse**, **iCIMS**, **eArcu**, **Broadbean**, **Paradox**.  
...
3. **Integration and calendar** constraints (include **PG-00009165** as **single GCC data point**; avoid naive substring **MENA** matching on free text).  
...
- **Win-loss source files read (this run):**  
  - `research/GCC/win-loss-interviews/Opportunity Detail.xlsx` (Tier 1: folder boundary; **598** rows on first sheet; **1** substantive GCC-relevant row after word-boundary keyword scan)  
- **Scratch dump produced (this run):**  
  - `research/GCC/win-loss-analysis/_scratch-winloss-sources.md` (from `python3 scripts/dump_research_folder_to_text.py research/GCC/win-loss-interviews -o research/GCC/win-loss-analysis/_scratch-winloss-sources.md --max-rows 3000`)  
