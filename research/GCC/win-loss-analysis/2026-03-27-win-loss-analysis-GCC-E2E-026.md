# Win-loss analysis (GCC) — GCC-E2E-026

## Executive summary

• **Source:** `research/GCC/win-loss-interviews/Opportunity Detail.xlsx` (598 rows, one sheet). Flattened via `scripts/dump_research_folder_to_text.py` to `research/GCC/win-loss-analysis/_scratch-winloss-sources-GCC-E2E-026.md` for this run.

• **Gulf GCC coverage:** After applying **107** regional filtering (Tier 1 folder include, row-level Gulf keywords across `Country Specific Gap Detail`, `Industry Specific Gap Detail`, `Gap Name`, `CI Notes`, `Pain point(s)`, `Proposed Solution`, `Gap Summary`, `Headliner Feature`, `Functionality Requirement`, `Opportunity`), **zero rows** reference Gulf Cooperation Council recruiting markets (Saudi, UAE, Qatar, Bahrain, Kuwait, Oman, nationalisation, Qiwa, Mudad, etc.).

• **Country column:** `Country Specific Gap Detail` is **empty for all 598 rows**, so row-level regional slicing per **107** Spreadsheet Filtering could not use that field. **`Industry Specific Gap Detail`** is populated on all rows; scanning it with Gulf keyword patterns still yields **no** Gulf market matches.
...
| `Industry Specific Gap Detail` non-empty | 598 | Scanned for Gulf keywords; **0** Gulf market hits |
...
Top themes for PMF triangulation as **Win-Loss (107):** **No Gulf-specific themes** from this extract; flag **107** column as **data gap** unless supplemental Gulf win-loss text is added. The single **Microsoft GCC High** row is **US** cloud compliance, not Gulf recruiting.
...
• **Win-loss source files read (this run):**
  ...
  • `research/GCC/win-loss-interviews/Opportunity Detail.xlsx` (Tier 1: folder `research/GCC/win-loss-interviews/`; 598 rows on `Sheet 1`; **Gulf-relevant rows after keyword scan and Microsoft GCC High exclusion: 0**)

  • `research/GCC/win-loss-analysis/_scratch-winloss-sources-GCC-E2E-026.md` (dump produced by `python3 scripts/dump_research_folder_to_text.py research/GCC/win-loss-interviews -o research/GCC/win-loss-analysis/_scratch-winloss-sources-GCC-E2E-026.md`)
...
• **Completed (UTC):** 2026-03-27T00:50:14Z
