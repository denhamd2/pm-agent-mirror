# Candidate Grid Redesign (2026R2)
Product Requirements Document  
March 2026

## Executive Summary

Workday is uniquely positioned to develop a next-generation candidate review experience that consolidates the recruiter's multi-tab workflow into a unified, high-performance interface. For 2026R2, we will deliver a redesigned Candidate Grid that integrates candidate summary, CV/resume, notes, timeline, and history into a seamless single-view experience—eliminating the context-switching tax that costs recruiters up to 2 hours daily when reviewing 100+ candidate profiles.

For our customers, this feature will reduce time-to-review by 40% through unified data presentation, improve decision quality with instant access to complete candidate context, and enable high-volume recruiting teams (Baker Hughes ~200 candidates/req, Accenture 8-10k applications/quarter) to process candidates faster without sacrificing thoroughness. Mobile-first recruiters will benefit from optimised touch targets and streamlined navigation—critical for GCC markets where 40%+ of recruiting activity occurs on handheld devices.

For Workday, this initiative will reduce recruiter abandonment to external tools (PowerBI, Excel exports for candidate tracking), strengthen retention amongst high-volume recruiting customers, and eliminate a key competitive gap versus regional ATS specialists (Talentera, ZenHR) who offer lighter, faster candidate review experiences. This positions Workday Recruiting as the system of record for operational recruiting—not just requisition management.

**Source Research:** GCC PMF Analysis v46 (March 2026) - Theme 3: Recruiter efficiency: grid, search, and AI matching  
...
| **How is our approach uniquely different from others?** | • **Unified candidate modal**: Single-pane view consolidating summary, CV (with inline rendering), notes, and timeline—no tab switching required<br>• **In-grid quick actions**: Prev/Next carousel navigation with keyboard shortcuts (arrow keys) for high-speed sequential review<br>• **Mobile-optimised**: Touch-friendly targets and swipe gestures for recruiters on handheld devices (40%+ of GCC recruiting traffic)<br>• **Contextual filtering**: Enhanced boolean search and saved filter sets for recruiter productivity<br>• **Workday-native**: Built into core Recruiting module—no third-party CRM or external tool required<br>• **Success metrics**: Time-to-review reduction (40% target), tab-switch events eliminated (baseline: 4 per candidate), recruiter satisfaction (NPS +15) |
...
• **Prev/Next Carousel Navigation**: Arrow buttons (top-right) and keyboard shortcuts (left/right arrow keys) allow sequential candidate review without closing modal—critical for batch screening workflows
...
- **Gap**: Workday's multi-tab architecture creates perception of complexity versus regional tools
...
- **Greenhouse** (SMB): Unified candidate view with Prev/Next carousel—feature parity expected

**Workday Differentiation Post-Redesign**: Combines enterprise-grade compliance (audit logs, encryption, Germany anonymisation support, GCC Arabic/RTL QA) with regional specialist speed (unified modal, mobile swipe)—unique positioning for GCC enterprise customers. **Incremental value over existing**: Workday already offers slide-out panel, CV preview column, and grid tabs (per Deployment Agent); v46 unified modal delivers **complete** summary+CV+notes+timeline in **one** surface with carousel (vs. panel = partial context; tabs = navigation tax).
...
- **AI-Assisted Candidate Matching**: "Similar candidates" and "Search across database" surfaces (Theme 3 from v46 research)
...
## Red Team Revision Summary (Step 3b)
...
**Document Status**: Revised (Post-Red Team)  
**Next Steps**: Discovery Brief (315) → Prototype (320) → Copy Review (319) → Figma Capture (330) → Backlog Refinement (400)  
