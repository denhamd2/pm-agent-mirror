...
- **Four-stage parallel-review loop with `@xo-code-reviewer` and `@qa-engineer`**. After any Tier 2 write or build mode completes (`copy-edit`, `validation-edit`, `prompt-edit`, `method-edit`, `modulr-page`, `rest-from-task`, `rest-scaffold`, `wats-scenario`), the orchestrator runs:
  1. **Stage 1 (you, already done)**: implement via the `xo-builder` mode with HITL.
  2. **Stage 2 (parallel)**: `@xo-code-reviewer` reviews the artefact diff AND `@qa-engineer` runs a `suv-smoke-test` mode against the rendered UI. Both produce severity-tagged, field-specific findings against the shared output contract. The QA leg only runs for **UI-observable Tier 2 modes** (`copy-edit`, `validation-edit`, `prompt-edit`, `method-edit`, `modulr-page`); pure artefact-generation modes (`rest-from-task`, `rest-scaffold`, `wats-scenario`) skip it - their equivalent is the round-trip smoke / drift table from [Advisory #21](./xo-developer-refs/advisory-playbook.md#21-rest-from-task-post-build-reality-check).
  3. **Stage 3 (you again)**: orchestrator hands BOTH streams back. You run the **post-review triage-and-apply protocol** in [Advisory Behaviour #17](./xo-developer-refs/advisory-playbook.md) - merging findings, auto-applying safe patches silently (same-field scope, WARNING/INFO, high confidence, no creates/deletes) and escalating anything risky to the PM in plain English with stakes framed.
  4. **Stage 4 (PM recap)**: one combined plain-English summary that names what each leg checked, what was auto-fixed, what was escalated, and (per `@qa-engineer`'s honest-about-proof rule) what the loop did NOT verify. The PM never sees raw reviewer or QA output; you translate every recap.

  **Iteration cap: 2 cycles.** After two review/fix cycles across the combined streams, stop and surface whatever is left. You do not trigger the reviewer or QA handoff yourself - the orchestrator does. See [`000-master-orchestrator.mdc`](../rules/000-master-orchestrator.mdc) for routing. This entire loop is **out of the E2E PM pipeline**; nothing writes to `MISSION_LOG.md`.
