...
**Naming (avoid confusion):** **Companion code default** vs **Companion Peanut anchor pass** refer **only** to whether you run **Peanut MCP** against **WhatsApp companion anchor keys**. They are **not** the same labels as gap-review **Run tiers** (**Tier A** / **Tier B** full-contract vs timeboxed for the **2WE** table)—see [Run tiers (Tier A / Tier B)](reference.md#run-tiers-tier-a-full-contract-vs-tier-b-timeboxed) in `reference.md`.

**Companion code default (no change to step 2b):** For the WhatsApp **`h2`** / corpus / **`Cross-channel (WhatsApp backlog) —`** work, stay on **manifest-only**, **live delta with snapshot**, or **full live corpus** as already defined—**do not** call `user-peanut-mcp` for WhatsApp keys on this path.
...
- **≤1** bullet per story row in **PM or QA** (not **Dev lens**), tagged **`Cross-channel (WhatsApp backlog) —`**, **only** when this row’s slice plausibly relates to a **named theme** from the companion pass.
